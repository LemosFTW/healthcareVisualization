        ...
        "entry": [
            ...
            },
            {
                "resource": {
                    ...
                    "valueQuantity": {"value": 0, "unit": "bpm"},
                }
            },
        ],
    }
...
def test_post_fhir_patient_returns_200():
    ...
    assert (
        _build_client()
        .post("/messages", json={"protocol": "fhir", "raw_payload": VALID_FHIR_PATIENT})
        .status_code
        == 200
    )
...
def test_post_fhir_patient_returns_stored_status():
    ...
    resp = _build_client().post(
        "/messages", json={"protocol": "fhir", "raw_payload": VALID_FHIR_PATIENT}
    )
...
def test_post_fhir_bundle_returns_stored_status():
    ...
    """
    resp = _build_client().post(
        "/messages", json={"protocol": "fhir", "raw_payload": VALID_FHIR_BUNDLE}
    )
    assert resp.json()["status"] == STATUS_STORED
...
def test_hl7_still_works_alongside_fhir_route():
    ...
    hl7_resp = client.post(
        "/messages", json={"protocol": "hl7v2", "raw_payload": hl7_payload}
    )
...
def test_post_malformed_fhir_returns_error_status():
    ...
    """
    resp = _build_client().post(
        "/messages", json={"protocol": "fhir", "raw_payload": MALFORMED_FHIR}
    )
    body = resp.json()
...
def test_post_fhir_with_anomaly_returns_warnings():
    ...
    resp = _build_client(ai_mock=ai_mock).post(
        "/messages", json={"protocol": "fhir", "raw_payload": VALID_FHIR_BUNDLE}
    )
    body = resp.json()
...
def test_fhir_decoder_is_separate_class_from_hl7_decoder():
    ...
    Then they must be distinct classes confirming FHIR support was added
    without modifying HL7 decoder
    """
...
def test_hl7_validator_is_unchanged_for_hl7_payloads():
    ...
    Then is_valid must be True confirming the validator was not modified
    for FHIR support
    """
